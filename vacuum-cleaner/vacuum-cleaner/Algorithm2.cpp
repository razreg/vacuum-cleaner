...
Direction Algorithm2::step() {
	...
	if (stepsToBypassWall > 0 || isReturnTripFeasable(abs(xPos) + abs(yPos) + 2 + MAX_BYPASS) 
		...
		else if (sensorInformation.dirtLevel == 0) {
...
Direction Algorithm2::keepCleaningOutOfDocking(SensorInformation& sensorInformation) {
...
Direction Algorithm2::returnToDocking(SensorInformation& sensorInformation) {
	Direction direction = Direction::Stay;
	if (!inDockingStation()) {
